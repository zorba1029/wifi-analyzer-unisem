...
object FlowsStatsTablesPartitionManager {
	def apply() = {
		new FlowsStatsTablesPartitionManager()
	}
}


class FlowsStatsTablesPartitionManager  extends PartitionManager {
	private val logger = LoggerFactory.getLogger(FlowsStatsTablesPartitionManager.getClass)
	private val DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd")
	private val appConfig = ConfigFactory.load()
	private val DB_NAME = appConfig.getString("analyzer.DB.db_name")
	private val TABLE_NAME = "flows_stats_hourly"


	def process(): Unit = {
		logger.info(s"FlowsStatsTablesPartitionManager ---> process() ------------------------- ")
	...
	private def processTablePartition(dt: LocalDate): Unit = {
		val partName = getPartitionNameForDate(dt)
		val partCond = getPartitionConditionForDate(dt.plusDays(1))

		//-- check if the Table exist & if not, create Table
		if (isTableExist() != true) {
			if (createTable(dt)) {
				logger.debug("processTablePartition() - flows_stats_hourly TABLE CREATED.")
			}
		} else {
			logger.debug("processTablePartition() - flows_stats_hourly TABLE ALREADY EXIST.")
		}
		...
		if (isTableExist()) {
			...
			if (hasASpecificPartition(partName) != true) {
				...
				if (exist != true) {
					...
					if (success) {
						...
						initFlowsStatsTableForDate(dt, stationIdList)
					}
					else {
						logger.error(s"processTablePartition() - ^^^^^^^^^^^^^^ [$partName]  ^^^^^^^^^^^^^^^^")
						logger.error("processTablePartition() -  1) flows_stats_hourly NOT CREATED. Exit...")
						logger.error("processTablePartition() - ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
						System.exit(-1)
					}
				}
				else {
					logger.error(s"processTablePartition() - Partition: [$partName]  ALREADY EXIST - INIT TABLE RECORDS..")
				...
				logger.error(s"processTablePartition() - Partition: [$partName]  ALREADY EXIST")
				val stationIdList = getStationIdList()
				initFlowsStatsTableForDate(dt, stationIdList)
			}
		}
			...
			logger.error(s"processTablePartition() - ^^^^^^^^^^^^^^  [$partName]  ^^^^^^^^^^^^^^^^")
			logger.error("processTablePartition() -  2) flows_stats_hourly NOT CREATED. Exit...")
			logger.error("processTablePartition() - ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
			System.exit(-1)
		}
	}
	...
	//		try {
	//			conn = DBDataSource.getConnection(-1)
	//			val preparedStmt: PreparedStatement = conn.prepareStatement(selectQry)
	//			val rs: ResultSet = preparedStmt.executeQuery
	//
	//			while (rs.next) {
	//				val stationId: String = rs.getString("station_id")
	//				//--logger.debug("Thread["+ -1 +"] getStationIdList, stationId = " + stationId);
	...
	private def initFlowsStatsTableForDate(day: LocalDate, stationIdList: List[(String,String)]): Boolean = {
		//--String pName = String.format("p%04d_%02d_%02d", day.getYear(), day.getMonth().getValue(), day.getDayOfMonth());
		...
		try {
			...
			for ((srcStationId, srcStationName) <- stationIdList) {
				for ((dstStationId, dstStationName) <- stationIdList) {
					if (srcStationId != dstStationId) {
						...
						if (rs.next != true) {
							//-- NOT EXIST -> INSERT
							logger.debug(s"ActorId[-1] ${TABLE_NAME} - INSERT RECORD($srcStationId[$srcStationName],$dstStationId[$dstStationName],$localDateStr) - $day, pName = $pName")
							...
							//-- ALREADY EXIST -> NO OP
							//--logger.debug(s"ActorId[-1] ${TABLE_NAME} - NO-OP RECORD($srcStationId[$srcStationName},$dstStationId[$dstStationName],$localDateStr) - $day, pName = $pName")
						}
					}
				}
			}
			conn.commit()
			logger.debug(s"ActorId[-1] ${TABLE_NAME} - initialized for $day, pName = $pName")
			return true
		} catch {
			case sqlEx: SQLException =>
				logger.error(s"ActorId[-1] *** SQLException: INSERT INTO ${TABLE_NAME} - ${sqlEx.getMessage}")
				return false
			case ex: Throwable =>
				logger.error(s"ActorId[-1]  Exception: ${ex.getMessage}")
				return false
	...
	private def createTable(startDT: LocalDate): Boolean = {
		...
		val createQry = s""" CREATE TABLE IF NOT EXISTS `${TABLE_NAME}` (
			                 ...
			                 | dst_station_id        VARCHAR(32)  NOT NULL COMMENT  'DestinationStation ID',
			                 ...
			                 |   COMMENT='Hourly Flows Statistics between Stations'
	...
	private def hasASpecificPartition(pName: String): Boolean = {
		...
		// 				AND TABLE_NAME = 'flows_stats_hourly'
	...
	private def addPartition(dbName: String, tblName: String, pName: String, pCond: String) = {
		val qry = s""" ALTER TABLE  ${dbName}.${tblName}
			           | ADD PARTITION (PARTITION ${pName}  VALUES LESS THAN ('${pCond}'))
			           | ;
							""".stripMargin

		logger.info(s"[DB] Add partition : $qry")
